                            ...
                            <div>
                                @foreach($relationType['entry'] as $relationEntry)
                                    <div class="bg-body-tertiary p-2 border py-2 mb-2 d-flex justify-content-between">
                                        <div>
                                            <span class="badge {{ $relationEntry['type'] === 'anime' ? 'bg-primary' : 'bg-secondary' }}">{{ $relationEntry['type'] }}</span>
                                            <h6>{{ $relationEntry['name'] }}</h6>
                                        </div>
                                        <div class="my-auto">
                                            <a href="#" class="btn {{ $relationEntry['type'] === 'anime' ? 'btn-primary' : 'btn-secondary' }}">
